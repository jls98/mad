...
def retrieve_dX(dp_input, dq_input, N, e):
    ...
    while(unfinished):
        ...
        # pq=N index to LSB 
        valid_combinations = [] # find all combinations of current candidates with bitlen of current index from LSB
        for p in candidates_p: 
            for q in candidates_q: 
                if check_pqN(p, q, N, index_count): valid_combinations.append((p, q))
        
        # remove invalid cands for p and q
        #for p in candidates_p: 
        #    found = False
        #    for pair in valid_combinations: 
        #        if p == pair[0]: found = True
        #    if not found: candidates_p.remove(p)        
        
        #for q in candidates_q: 
        #    found = False
        #    for pair in valid_combinations: 
        #        if q == pair[1]: found = True
        #    if not found: candidates_q.remove(q)        
            
        # apply cands for p and q if only 1 left
        #if len(candidates_p)==1: candidate_p=bin(candidates_p[0])[2:]
        #if len(candidates_q)==1: candidate_q=bin(candidates_q[0])[2:]
        
        # check dp equation
        valid_dp_combination = []
        for p, q in valid_combinations: # valid p and q
            for dp in candidates_dp:    # check candidates for dp
                for kp in candidates_kp:# and kp
                    if n_bits((e * dp), index_count) == n_bits(kp*(p-1)+1, index_count): # pq=N checked already, checking e*dp equation
        ...
        if not bit_retrieved: print("fail!!!")
