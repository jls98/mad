 ...
 */
static void
sqr_mod (mpi_ptr_t xp, mpi_size_t *xsize_p,
         mpi_ptr_t rp, mpi_size_t rsize,
         mpi_ptr_t mp, mpi_size_t msize,
         mpi_ptr_t tp)
{
  if( rsize < KARATSUBA_THRESHOLD )
      ...
      mpih_sqr_n( xp, rp, rsize, tp );
  }
  if (rsize * 2 > msize)
    {
      mpihelp_divrem (xp + msize, 0, xp, rsize * 2, mp, msize);
      *xsize_p = msize;
    }
  else
     *xsize_p = rsize * 2;
...
mpi_powm (MPI res, MPI base, MPI expo, MPI mod)
{
  ...
  /* Main processing.  */
  {
    ...
    tspace = mpi_alloc_limb_space( 2 * (msize + 1), msec );
    ...
    for (;;)
      if (e == 0)
      ...
      else
        {
          ...
          for (j += W - c0; j; j--)
            {
              sqr_mod (xp, &xsize, rp, rsize, mp, msize, tspace);
              tp = rp; rp = xp; xp = tp;
              rsize = xsize;
            }
