...
int monitor_modified(void *p, uint32_t period, uint64_t maxwait, int index, uint64_t *timestamps)
{
	...
	{	
		...
		if (timestamps[index] < 100) return 1;
...
int main(int ac, char **av){
	...
	//printf("period %li\n", period);
	uint64_t timestamps[AMOUNT_SUCCESS] = {0};
	...
	while((rdtsc() - start) < CYCLE_LIMIT){
		counter+=monitor_modified(location, period, CYCLE_LIMIT/100, counter, timestamps);
		if (counter>=AMOUNT_SUCCESS)
		{
			break;
			// success
		}
	...
	for (counter=0; counter<AMOUNT_SUCCESS; counter++) printf("%lu\n", timestamps[counter]);
