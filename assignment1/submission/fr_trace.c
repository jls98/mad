...
reload times of p1 and p2 are stored in trace[0] and trace[1], respectively. More generally, the 
results of the ith iteration are stored in trace[2*i] and trace[2*i+1]. You can assume that the array 
trace is large enough to store all the results.

Use the function to build a binary fr_trace <file> <offset1> <offset2> that traces accesses to offset1
 and offset2 in the file file. Use a period of 10000 cycles and collect 1000 such samples. The output 
 is 1000 lines, each with two numbers -- the probe times of offset1 and offset2. 

To test fr_test, use our rattle program /home/victim/rattle. rattle uses the same arguments as 
fr_trace, and generates a repeated access pattern.  To use, run rattle in the background, then run 
fr_trace with the same arguments, and submit the output trace as trace.out.  Additionally, plot the 
collected trace and include the resulting figure in the answers PDF. See below for an example of such 
a trace.
*/
...
int main(int ac, char **av){
	...
	printf("p1 p2\n");
	for (int i=0;i< COUNT;i++)printf("%4u %4u\n",traces[2*i], traces[2*i+1]);
